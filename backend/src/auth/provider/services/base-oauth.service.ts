  ...
  public async findUserByCode(code: string): Promise<TypeUserInfo> {
    ...
    if (!tokenRequest.ok) {
      throw new BadRequestException(
        'Was unable to get user data. Please check access token'
      )
    }
    ...
    if (!tokens.access_token) {
      throw new BadRequestException(
        'No access token provided. Make sure that authorization code is valid.'
      )
    }
    ...
    if (!userRequest.ok) {
      throw new UnauthorizedException(
        'Cant get user data. Please check access token'
      )
    }
